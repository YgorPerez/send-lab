...
//
// 1. WHICH ATHLETE IS LOOKING AT THIS PAGE
// ----------------------------------------
// Three of them are, and they need three different sentences:
//
//   * one with no account, who has never signed in anywhere;
//   * one whose session ended, whose training is all still on this device;
//   * one who is signed in already and arrived here by tapping the wrong thing.
//
// The session cannot tell the first two apart — it is absent for both. What
// tells them apart is whether this device is still holding a record, which is
// `heldAccounts()` in `store/collections.ts`. That matters more here than
// anywhere else in the app, because of what #24 decided: **session expiry never
// clears the local store or the queue.** Only an explicit sign-out, made online
// and after the queue has drained, clears anything. So the athlete standing in
// front of an expired session still has every set they logged and every write
// that has not been sent yet — and a sign-in screen that reads like a fresh
// install is telling them, wrongly, that it is all gone.
//
 ...
 *  other two say who the page thinks is reading it. */
export type LoginNotice = 'offline' | 'returning' | 'first-run';
 ...
 * `returning` and `first-run` are facts about the reader, and the two are
 * independent. An expired athlete standing in a gym basement is exactly the
 * athlete the reassurance is written for — suppressing it there would drop the
 * line in the one state that most needs it.
 */
export function resolveLogin({ account, online, held }: LoginInput): LoginScreen {
	const signedIn = account !== null;
	if (signedIn) return { signedIn, notices: [] };
	const who: LoginNotice = held.length > 0 ? 'returning' : 'first-run';
	return { signedIn, notices: online ? [who] : ['offline', who] };
}
