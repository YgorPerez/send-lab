// Display formatting for prescriptions and logged sets.
//
// The SvelteKit app kept these in `$lib/plan` + `$lib/units`, neither of which
// survived into the rebuild. They are re-derived here rather than ported whole:
// `units.ts` existed to convert kg↔lb and mm↔in against a stored preference,
// and the rebuild has no preferences store yet, so everything below is
// canonical units only. When the real settings screen lands, this is the seam
// that grows the conversion back.
//
...
/** The five wellness dimensions, in the order the score reads them. */
